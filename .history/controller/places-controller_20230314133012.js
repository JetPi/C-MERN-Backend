...
async function getPlaceByUser(req, res, next) {
  ...
  if (!userWithPlaces || userWithPlaces.places.length === 0) {
    return next(new HttpError("Couldn't find user's places, either the user has none, or the user doesn't exist", 404));
  }
...
async function modifyPlace(req, res, next) {
  ...
  try{
    ...
    if(place.creator.toString() !== req.userData.userId){
      const error = new HttpError(
        "You aren't allowed to edit this place",
        401
      );
      return next(error);
    }
...
async function deletePlace(req, res, next) {
  ...
  if(place.creator.toString() !== req.userData.userId){
    const error = new HttpError("No tienes permitido borrar este lugar", 401);
    return next(error);
  }
